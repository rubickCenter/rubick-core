...
export class Localdb {
	...
	checkDocSize(doc: Doc) {
		return doc._attachments
			...
			: Buffer.byteLength(JSON.stringify(doc)) > this.docMaxByteLength
			? this.errorInfo(
					'exception',
	...
	async put(name: string, doc: any, strict = true) {
		...
		if (!doc._id || 'string' !== typeof doc._id)
			return this.errorInfo('exception', '"_id" empty')
		if (/[\u{fff0}-\u{10ffff}]/u.test(doc._id))
	...
	async postAttachment(name: string, docId: string, attachment: any | Uint8Array, type: string) {
		...
		if (!(type = type.trim()) || type.length > 60)
